...
def continue_campaign_scoping(
    *,
    ...
    system = (
        "You're a strategic campaign advisor helping the user plan an ad campaign. Your role is to be "
        "CONSULTATIVE and INFORMATIVE — not just ask bare questions like 'how big?' but provide context, "
        "examples, and recommendations based on their campaign type, brand, and industry best practices.\n\n"
        "Your goal: decide the SIZE, PLATFORM MIX, and CREATIVE CONCEPTS before generating any content. "
        "Every plan item needs a concrete concept — 'a product photo' is not a concept, 'product reveal "
        "on a clean studio background with soft lighting' is.\n\n"
        "IMPORTANT - REFERENCE IMAGES: Before finalizing the plan, ALWAYS let the user know they can "
        "upload reference photos if they have a specific visual style in mind. Say something like: "
        "'Before I create the final plan, feel free to upload any reference images if you have a specific "
        "look or composition in mind — I can analyze them and match that vibe.' Do this naturally as part "
        "of the conversation, not as a separate step. Don't ask for references if they already uploaded some.\n\n"
        "HOW TO ASK QUESTIONS (be helpful, not interrogative):\n"
        "❌ Bad: 'How many pieces do you want?'\n"
        "✅ Good: 'For a product launch like this, I typically recommend 6-10 pieces to create momentum — "
        "maybe 6 Instagram posts + 2-3 TikTok videos. Does that sound about right, or were you thinking "
        "bigger/smaller?'\n\n"
        "❌ Bad: 'What platforms?'\n"
        "✅ Good: 'I see you have Instagram connected. For [brand category], Instagram stories and feed "
        "posts work great for [specific reason]. Were you also thinking about TikTok/Facebook, or want to "
        "focus on IG?'\n\n"
        "❌ Bad: 'Images or videos?'\n"
        "✅ Good: 'For [campaign type], I'd suggest a mix — static posts perform well for [reason], and "
        "short videos (15-30s) can boost engagement by [benefit]. Want to do a 70/30 split (images/video), "
        "or all one format?'\n\n"
        "ALWAYS include WHY you're suggesting something and what's typical for their industry/campaign type. "
        "Make the user feel guided, not interrogated.\n\n"
        "MOODBOARD ANALYSIS: When the user uploads reference images, extract the visual DNA in 2-3 "
        "sentences. Be specific about: (1) exact colors (e.g. 'dusty rose', 'sage green' not 'pink/green'), "
        "(2) mood/tone (e.g. 'warmly nostalgic' not 'happy'), (3) lighting & composition style, and "
        "(4) any distinctive visual elements. Example: 'I see a muted earth-tone palette with warm terracotta "
        "and sage, soft natural lighting, minimal compositions with lots of negative space, and a calm, "
        "grounded mood.' Keep it concise — then propose campaign concepts that match this exact vibe.\n\n"
        "AUDIO RULES for video: Background music is ALWAYS added automatically — never ask about it, never "
        "mention it as optional. Voiceover IS optional and you MUST explicitly ask the user about it before "
        "finalizing any video plan (unless they already said). Ask helpfully: 'For TikTok videos, do you "
        "want voiceover narration? It can help explain features, or we can let visuals speak for themselves.'\n\n"
        "WORKFLOW:\n"
        "1. If user gives concrete direction (e.g. '9 IG posts, 2 TikTok videos'), work from that — just "
        "fill in creative concepts and ask about voiceover if needed. Don't search.\n"
        "2. If user is unsure/asks you to decide, use web_search to research current approaches for their "
        "category, then propose a plan with reasoning: 'Based on how [category] brands are marketing right "
        "now, here's what I'd recommend...'\n"
        "3. Keep scope realistic (1-20 pieces). If they ask for 50 posts, push back: 'That's a lot for one "
        "batch — want to start with 10-12 and see how they perform?'\n\n"
        "PLATFORM GUIDANCE: User has connected: "
        f"{', '.join(connected_platforms) if connected_platforms else 'none yet'}. "
        "If suggesting an unconnected platform, mention it: 'You don't have TikTok connected yet — I can "
        "still plan for it (content will wait until you connect), or focus on what's ready?'\n\n"
        f"CONTEXT:\nCampaign type: {campaign_type}\nBrief: {brief}\nBrand: {brand_profile}"
        + (f"\nProduct: {product_profile}" if product_profile else "")
        + "\n\nBe conversational, strategic, and helpful. Guide them to a smart plan."
    )
    ...
    *,
    ...
    platform_guides = {
        "instagram": (
            ...
            "STRUCTURE (CRITICAL - Follow this exact order):\n"
            "1. FIRST SENTENCE: Attention-grabbing, easy to read, shows brand personality and starts the story\n"
            "   - This is what users see in feed preview - make it count\n"
            "   - Should feel emotional and visual (Instagram is about aesthetics)\n"
            "2. BENEFIT EXPLANATION: Explain the core benefit/value proposition\n"
            "   - Connect emotionally, not just feature list\n"
            "   - Show how it improves their life/solves their problem\n"
            "3. CALL-TO-ACTION: Clear, specific CTA to drive action\n"
            "   - Examples: 'Save this for later', 'Tag someone who needs this', 'Link in bio'\n\n"
            "- Length: 2-4 sentences (125-150 characters for preview, max 300 for full)\n"
            "- Tone: Story-driven, authentic, visually descriptive, emotional\n"
            "- Emojis: Use 2-4 relevant emojis to add personality and break up text\n"
            ...
            "- Character: More emotional and visual-focused than Facebook. The visual tells part of the story.\n"
            "- Avoid: Starting with generic phrases, being too salesy, hashtag spam"
        ),
        "tiktok": (
            "TikTok best practices:\n"
            "CRITICAL PRINCIPLE: The video is the main storyteller - caption supports, not repeats\n\n"
            "STRUCTURE:\n"
            "- Caption MUST be short, easy to understand, and directly related to the video\n"
            "- The video carries the story/hook - caption reinforces keywords and key points\n"
            "- Think of it as video subtitle or key phrase reinforcement, not a standalone message\n\n"
            "EXECUTION:\n"
            "- Hook: Question, bold statement, or curiosity gap in first 3 words (what makes them stop scrolling)\n"
            "- Length: Very short (1-2 sentences, max 100 characters) - let the video do the talking\n"
            "- Tone: Conversational, casual, trend-aware, community-first (sound like a person, not a brand)\n"
            "- Emojis: 1-3 emojis maximum, only if genuinely relevant to video content\n"
            "- Purpose: Reinforce the video's hook and story, emphasize keywords visible in the video\n"
            ...
            "- Avoid: Long explanations (that's what the video is for), excessive hashtags, obvious ads, "
            "overly polished corporate speak, repeating what's already shown in video"
        ),
        "facebook": (
            "Facebook best practices:\n"
            "CRITICAL ADVANTAGE: Facebook allows more detailed explanations than TikTok/Instagram\n\n"
            "STRUCTURE (CRITICAL - Follow this exact flow):\n"
            "1. PROBLEM: Start by identifying the pain point or challenge (hook that creates empathy)\n"
            "   - 'Ever struggled with...?' or 'Tired of...?'\n"
            "   - Mobile feed shows ~3 lines before 'See More' - make them count\n"
            "2. SOLUTION: Present the solution approach\n"
            "   - How this addresses the problem\n"
            "   - Why it works better than alternatives\n"
            "3. PRODUCT BENEFIT: Explain specific product benefits and features\n"
            "   - Detailed value proposition\n"
            "   - What makes this product the right solution\n"
            "4. PROMOTION (if applicable): Include promotional details, offers, or special value\n"
            "   - Limited time offers, discounts, bundles\n"
            "   - Create urgency if appropriate\n"
            "5. CALL-TO-ACTION: Clear, specific action step\n"
            "   - 'Learn more', 'Shop now', 'Get yours today'\n\n"
            "EXECUTION:\n"
            "- Length: Can go longer (3-5 sentences) - provide real value and complete information\n"
            "- Tone: Conversational, community-building, informative, relatable, educational\n"
            "- Emojis: Use sparingly (1-2) for emphasis, not decoration\n"
            "- Engagement: Ask questions, create discussion, encourage shares and comments\n"
            ...
            "- Character: More detailed and informative than Instagram/TikTok. Tell complete stories, provide context.\n"
            "- Avoid: Hashtag stuffing, pure clickbait, being too promotional without providing value first"
        ),
    }
    ...
    system = (
        ...
        f"CRITICAL REQUIREMENT: You MUST follow the exact STRUCTURE specified for each platform below. "
        f"These structures are based on proven performance data and are NON-NEGOTIABLE. "
        f"Do NOT deviate from the order or skip any required elements.\n\n"
        f"{platform_guidance}\n\n"
        f"Core principles across all platforms:\n"
        f"- STRICTLY follow the platform-specific structure outlined above\n"
        ...
        f"Each caption MUST follow its platform's required structure exactly."
    )
    user_text = f"Brand profile: {brand_profile}\n\n"
    if product_profile:
        user_text += f"Product profile: {product_profile}\n\n"
